...
class App extends Component {
  ...
  startTimer = () => {
    this.timerID = setInterval(() => {
      const {time, isGameInProgress} = this.state
      if (isGameInProgress && time > 0) {
  ...
  onClickPlayAgain = () => {
    this.setState(initialState, () => {
      this.startTimer() // Restart timer on play again
    })
  }

  renderImageList = () => {
    const {activeImageId, activeTabId} = this.state
    const activeImage = imagesList.find(img => img.id === activeImageId)
    const filteredImageList = this.getFilteredImageList()
      ...
      <div className="game-container">
          ...
          {tabsList.map(tabDetails => (
            <li className="tab-item-container" key={tabDetails.tabId}>
              <button
                ...
                onClick={() => this.onClickTabItem(tabDetails.tabId)}
              >
        ...
        <ul className="images-list-container">
          {filteredImageList.map(imageDetails => (
            <li className="image-item-container" key={imageDetails.id}>
              <button
                id={imageDetails.id}
                type="button"
                className="image-buttons"
                onClick={() => this.onClickImageItem(imageDetails.id)}
              >
                <img
  ...
  render() {
    const {score, time, isGameInProgress} = this.state
    ...
    return (
      <div className="app-container">
        ...
        <div className="game-body">
          {isGameInProgress ? this.renderImageList() : this.renderScoredCard()}
        </div>
      </div>
    )
  }
